...
# Reasoning-model providers (e.g. DeepSeek's deepseek-v4-flash) spend part of
# max_tokens on hidden reasoning before writing the visible answer. Keep this
# generous or short replies silently come back empty.
...
_SCOPE_RULES_VI = (
    ...
    "không đoán.\n"
    "- Không khẳng định chắc chắn thiên tai sẽ xảy ra — chỉ nói theo mức rủi ro/cảnh báo có trong dữ liệu.\n"
    "- Không tự bịa địa điểm sơ tán, số điện thoại liên hệ, hay hướng dẫn trái với cảnh báo chính thức "
    "(NCHMF) trong dữ liệu.\n"
...
_SCOPE_RULES_HMONG = (
    ...
    "- Txhob hais tseeb tias yuav muaj xwm ceev tshwm sim — hais raws li qib kev txhawj xeeb hauv ntaub ntawv xwb.\n"
    "- Txhob tsim qhov chaw khiav dim los sis lus qhia uas tsis phim raws li NCHMF cov lus ceeb toom tseeb.\n"
...
def _format_context(context: dict | None) -> str:
    ...
    overall = context.get("overall_risk") or {}
    bulletin = context.get("bulletin") or {}

    lines = [f"DỮ LIỆU THẬT CHO XÃ {commune.get('name', '')} (ngày 0 = hôm nay, ngày 1 = ngày mai, v.v.):"]
    lines.append(f"Mức rủi ro tổng quát hiện tại: {overall.get('label', 'chưa rõ')}.")
    if current:
        lines.append(
            f"Thời tiết hiện tại: {current.get('condition', 'chưa rõ')}, nhiệt độ {current.get('temperature_c')}°C "
            f"(cảm giác {current.get('apparent_temperature_c')}°C), độ ẩm {current.get('humidity_percent')}%, "
            f"gió {current.get('wind_speed_kmh')}km/h."
        )
    for row in forecast:
        parts = [
            f"- Ngày {row.get('day_index')} ({row.get('date')}): {row.get('condition')}, "
            f"nhiệt độ {row.get('temp_min_c')}-{row.get('temp_max_c')}°C, "
            f"mưa {row.get('rain_sum_mm')}mm, mức rủi ro {row.get('risk', {}).get('label')}"
        ]
        if row.get("landslide"):
            parts.append("CÓ cảnh báo sạt lở đất chính thức từ NCHMF cho ngày này")
        if row.get("flash_flood"):
            parts.append("CÓ cảnh báo lũ quét chính thức từ NCHMF cho ngày này")
        lines.append(", ".join(parts) + ".")
    if bulletin.get("text"):
        lines.append(f"Bản tin đầy đủ hiện có: {bulletin['text']}")
    return "\n".join(lines)
...
def spell_correct(text: str, lang: str) -> str:
    ...
    return _call(messages, max_tokens=600, temperature=0.1, _label="llm_spell")
...
def chat_text_stream(history: list, user_message: str, language: str = "vietnamese", context: dict | None = None):
    ...
    messages.extend(history[-20:])
    messages.append({"role": "user", "content": user_message})
    stream = llm_provider.get_client().chat.completions.create(
        model=llm_provider.get_model(),
        messages=messages,
