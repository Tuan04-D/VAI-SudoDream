...
const UNIT: Record<"temp" | "precip", string> = { temp: "°C", precip: "mm" };
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.addLayer({
        ...
        type: "line",
        source: "communes",
        filter: ["in", ["get", "risk_level"], ["literal", ["cao", "nguy_hiem"]]],
        paint: {
          "line-color": ["match", ["get", "risk_level"], "nguy_hiem", RISK_COLOR_HEX.nguy_hiem, "cao", RISK_COLOR_HEX.cao, "#000"],
          "line-width": 2.4,
        },
      });
      ...
      map.on("mousemove", "communes-fill", (e) => {
        ...
        const value = row ? (metric === "temp" ? row.temp_downscaled : row.precip_downscaled) : null;
        popup
          .setLngLat(e.lngLat)
          .setHTML(
            `<div class="text-xs font-semibold text-ink">${name}</div>` +
              (value !== null
                ? `<div class="font-data text-sm font-bold text-primary">${value}${UNIT[metric]}</div>`
                : "")
          )
          .addTo(map);
      });
  ...
  function decorate(
    ...
    return {
      ...
      features: fc.features.map((f) => {
        ...
        const row = values[id];
        const raw = row ? (metricKey === "temp" ? row.temp_downscaled : row.precip_downscaled) : lo;
        const norm = hi === lo ? 0.5 : Math.max(0, Math.min(1, (raw - lo) / (hi - lo)));
        ...
        return {
          ...
          properties: {
            ...f.properties,
            value_norm: norm,
            risk_level: row?.risk_level ?? "thap",
