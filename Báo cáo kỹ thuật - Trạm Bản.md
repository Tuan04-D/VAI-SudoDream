...
Báo cáo này trình bày toàn bộ hệ thống: từ bài toán xã hội đặt ra và định hướng thiết kế của dự án, đến kiến trúc kỹ thuật chi tiết của từng module — trọng tâm là module agent voice (kiến trúc mô hình, quy trình tinh chỉnh ASR tiếng H'Mông, tổng hợp giọng nói, kiến trúc hội thoại thời gian thực, kho tri thức và các ràng buộc an toàn AI) — cho đến bộ máy đánh giá rủi ro có định lượng bất định thật bằng ensemble đa mô hình, phần mềm backend, frontend, cơ sở dữ liệu, lộ trình khả thi triển khai thực địa, và cách toàn bộ hệ thống được tích hợp thành một sản phẩm hoàn chỉnh, có thể dùng ngay. Báo cáo cũng ghi lại thử nghiệm hiệu chỉnh địa hình như một kết quả nghiên cứu offline chưa được bật trong runtime hiện tại. Báo cáo kết thúc bằng đánh giá trung thực về giới hạn và lộ trình mở rộng sang các tỉnh miền núi khác và các ngôn ngữ dân tộc thiểu số khác.
...
│   · Downscaling địa hình: thử nghiệm offline, chưa bật runtime    │
│     — 2.3.3                                                       │
└───────────────┬────────────────────────────────┬──────────────────┘
                │                                │
┌───────────────▼────────────────┐  ┌────────────▼─────────────────┐
...
Về xác thực: người dân, cán bộ xã và quản trị viên đăng nhập bằng số điện thoại và mật khẩu được băm một chiều bằng Argon2. Backend phát access JWT ngắn hạn và refresh token xoay vòng trong cookie HttpOnly; phiên có thể bị thu hồi thông qua token version. Kiểm soát quyền được thực hiện ở phía máy chủ theo ba vai trò `resident`, `official`, `admin`, đồng thời giới hạn cán bộ trong đúng phạm vi xã được phân công. OTP, khôi phục tài khoản và rate limiting phân tán vẫn là các hạng mục cần bổ sung trước khi triển khai diện rộng.
...
Cảnh báo thiên tai (sạt lở, lũ quét) được trình bày rõ là dữ liệu ở phạm vi cấp xã vì nguồn NCHMF hiện chưa cung cấp độ phân giải đến từng hộ dân. Giao diện không diễn giải giá trị thời tiết cấp xã thành một dự báo chính xác giả ở từng toạ độ.
...
Bản đồ dành cho cán bộ hiển thị **toàn tỉnh**, giữ bối cảnh địa lý giữa các xã và đồng bộ với ngày đang chọn trong dự báo 5 ngày. Các lớp nhiệt độ và lượng mưa hiện dùng giá trị gốc từ nguồn dự báo; giao diện không gắn nhãn “đã hiệu chỉnh địa hình” khi module downscaling chưa được đưa vào runtime.
...
Dữ liệu production được lưu trên MongoDB. Các collection chính gồm `communes`, `users`, `auth_sessions`, `chat_messages`, `alerts`, `alert_views`, `alert_deliveries` và `audit_logs`; index được tạo tự động khi backend khởi động. Quan hệ nghiệp vụ được giữ bằng ID và kiểm tra tại tầng service: người dùng gắn với xã, cảnh báo gắn với xã/người phát, trạng thái đã xem gắn với cặp cảnh báo–người dân, còn outbox giao nhận tách khỏi bản ghi cảnh báo để có thể retry mà không tạo cảnh báo trùng.
...
- **SMS**: backend đã có durable outbox và bridge giao nhận; kênh chỉ được bật khi cấu hình gateway, hiện chưa phải điều kiện bắt buộc của bản demo web.
...
| 2 — Mở rộng | Toàn bộ 45 xã/phường tỉnh Điện Biên | Tích hợp kênh Zalo và loa truyền thanh xã (2.8); bổ sung OTP, khôi phục tài khoản và rate limiting phân tán | Vận hành ổn định toàn tỉnh, có số liệu định lượng về thời gian cảnh báo đến người dân so với kênh truyền thống |
